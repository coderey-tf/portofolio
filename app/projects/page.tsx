...
const getProject = async (filter: string, textSearch: string) => {
  // "use server";
  const query = `*[_type == 'project' ${
    filter == "all" || !filter ? "" : `&& status match '${filter}' `
  }
  ${
    textSearch == "" || !textSearch
      ? ""
      : `&& status match '${textSearch}' || title match '${textSearch}*' || overview match '${textSearch}' || techstack match '${textSearch}'`
  }
]
    {
      title,
...
export default async function Projects({
  ...
  return (
    <div className="divide-y divide-gray-500 dark:divide-gray-700">
      ...
      <div>
        <div className="grid gap-4 grid-cols-2  md:grid-cols-3 md:gap-8 pt-8">
          <div className="col-span-1 md:col-span-2 inline-flex justify-end items-center gap-1">
            <Filter datas={datas} selected={selectedSearch || ""} />
          </div>
          <div className="col-span-1">
            <Search />
          </div>
        </div>
        <div className="flex flex-col md:flex md:flex-row  items-center gap-8 mt-8">
          {data.length == 0 ? (
            <div className="text-xl md:text-2xl font-semibold text-black dark:text-white mx-auto my-32">
            ...
            <>
              {data.map((project) => {
                return (
                  <article
                    key={project._id}
                    className="h-[425px]  w-[340px] dark:border-zinc-600 rounded-lg border border-gray-400 bg-white shadow-lg dark:bg-black  shadow-gray-500 hover:scale-105 transition-all duration-300 "
                  >
                    <div className="h-64 md:h-56 w-full relative">
                    ...
                    <div className="p-4 space-y-2 sm:p-6">
                      ...
                      <ScrollArea className="py-2">
                        ...
                        <div className="flex overflow-x-auto items-center gap-2 pt-2">
